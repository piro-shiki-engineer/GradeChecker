...
function getGradesTable(): HTMLTableElement | null {
    ...
    return tables[1] as HTMLTableElement;
}

function parseGrades(table: HTMLTableElement): Grade[] {
    const grades: Grade[] = [];
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (cells.length >= 13) {  // ヘッダーを除く行の列数
            grades.push({
                ...
                credit: parseFloat(cells[6].textContent?.trim() || '0') || 0,
                grade: cells[7].textContent?.trim() || '',
                category: cells[8].textContent?.trim() || '',
                gp: parseFloat(cells[9].textContent?.trim() || '0') || 0,
                rankingRate: cells[10].textContent?.trim() || '',
                numStudents: parseInt(cells[11].textContent?.trim() || '0') || 0,
...
function matchSubject(subjectName: string, subjectConfig: SubjectConfig): boolean {
    return subjectConfig.genres.some(genre => subjectName.includes(genre));
}

function isInProgress(grade: Grade): boolean {
    return grade.grade === '履修中';
}

function isGradeConfirmed(grade: Grade): boolean {
    return !isInProgress(grade) && grade.passFail === '合';
}

function shouldIncludeGrade(grade: Grade, includeInProgress: boolean): boolean {
    return isGradeConfirmed(grade) || (includeInProgress && isInProgress(grade));
}

function checkDetailedRequirements(grades: Grade[], courseRequirements: CourseRequirement, includeInProgress = false): GraduationRequirementsResult {
    const results: GraduationRequirementsResult = {
    ...
    // 履修中の科目と確定済みの科目を分離
    const confirmedGrades: Grade[] = [];
    const inProgressGrades: Grade[] = [];
...
function displayResults(): void {
    ...
    const resultsElement = document.getElementById('graduation-requirements-result');
...
function createResultHTML(results: GraduationRequirementsResult): string {
    let html = `
...
function addToggleEventListeners(): void {
    document.querySelectorAll('.toggle-details').forEach(button => {
        button.addEventListener('click', function(this: HTMLButtonElement) {
            const subjectList = this.nextElementSibling as HTMLUListElement;
...
function fetchCourseRequirements(selectedCourse: string, grades: Grade[]): void {
    fetch(chrome.runtime.getURL('data/course-requirements.json'))
        .then(response => response.json())
        .then(data => {
            const courseRequirements = data.courses.find((course: CourseRequirement) => course.id === selectedCourse);
